...
function Joystick:isPressed(query)
   if profile.joystick.axis then
      local pressed = self.device:xyState(profile.joystick.axis):direction(query):pressed()
      if pressed then
	 return true
      end
   end
   local key = profile.joystick[query]
...
function Joystick:isJustPressed(query)
   if profile.joystick.axis then
      local state = self.device:xyState(profile.joystick.axis):direction(query)
